...
# ══════════════════════════════════════════════════════════
# API KEY — secrets first, manual input as fallback
# ══════════════════════════════════════════════════════════

# 1. Try Streamlit secrets (set in Streamlit Cloud dashboard)
...
with st.sidebar:
    ...
    if _secret_key:
        # Key is pre-configured — show a status badge, no input needed
        api_key = _secret_key
        st.markdown("""
        <div style='background:#0e1e0e; border:1px solid #2a4a2a; border-radius:4px;
                    padding:0.75rem 1rem; font-size:0.68rem; color:#7ab87a;
                    letter-spacing:0.08em; margin-bottom:0.8rem;'>
            ✓ API key configured
        </div>
        """, unsafe_allow_html=True)
    else:
        # No secret set — let the user paste their key manually
        ...
        st.markdown("""
        <div style='margin-top: 0.8rem; background: #13131e; border-left: 3px solid #c9a96e;
    ...
    <div style='font-size: 0.62rem; color: #3a3530; line-height: 1.9;'>
        ...
        1. Configure jewelry attributes<br>
        2. Click Predict Price<br>
        3. Get AI-powered market analysis
    </div>
    """, unsafe_allow_html=True)
...
with col_form:
    ...
    has_key      = bool(api_key and len(api_key.strip()) > 20)
